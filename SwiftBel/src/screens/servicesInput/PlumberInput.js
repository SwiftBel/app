...
import { getElectricianDetails, getMovingInputDetails, getPlumberDetails, getPressureWashDetails } from '../../store/actions/Profile.action'
...
const Electrician = props => {
    ...
    const onselect = async () => {
        ...
        res = await dispatch(getPlumberDetails(values))

        if (res.status) {
            // handleSnapClose()
            props.navigation.navigate('getPrice', { type: "Plumbers" })
        }
        else {
            Alert.alert(
                "Error",
