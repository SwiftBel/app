import { condo, house, townhouse } from "../../../assets"

export const listOfServicerResolved = [
    "• Flickering lights (LED lights reduce this problem)",
    "• Bulb burnouts (LED light transformers)",
    "• Circuit breaker keeps tripping or fuse blows",
    "• Switches and outlets shock when touched",
    "• Electrical outlet is dead",
    "• Very high electric bill (diagnostic service)",
    "• Faulty light switch",
    "• Broken ground circuit fault interrupter (GFCI outlet or breaker)",
    "• Incessant Buzzing Noises",
    "• Burning smells",
    "• Arcs or sparks",
    "• Loose power outlets",
    "• Install light fixture",
    "• Wires left hanging after renovations"

]

export const listOfHVACResolved = [
    " • System requires maintence",

    "• Dirty filters",

    "• Pilot or ignition problem",

    "• Thermostat malfunction",

    "• Mechanical problems (belts, bearings and motors wear out)",

    "• Furnace noises",

    "• Blow fuses and tripped circuit breakers",

    "• Dirty condenser or evaporator coils (AC) ",

    "• Blower runs continupusly",

    "• Water leaks (tends to be AC but can be high efficiency furnace)"
]

export const listOfACResolved = [
    " • Heat pump/AC not working",

    "• Condensing unit fan running; the indoor fan not working",

    "• Heat pump/AC not cooling as expected",

    "• The furnace or air handler is leaking water",

    "• Air conditioner indoor coil freeze-ups",

    "• Heat pump outdoor coil freeze-ups",

    "• The AC is over-cooling your home",

    "• System making unusual noise",

    "• Heat pump not heating",

    "• AC/heat pump outside unit not running",

    "• AC smells like chemicals"
]

export const listOfPlumberResolved = [
    "• Dripping faucets",

    "• Slow draining sink",

    "• Clogged bath ors howerdrain",

    "• Clogged toilet",

    "• Running toilet",

    "• Faulty water heater",

    "• Low water pressure",

    "• Jammed garbage disposal. Illegal in many places ",

    "• Leaky pipes",

    "• Sewer system backup"
]
export const ElevatorOptions=['Shared elevator','Dedicated elevator','Stairs','Ground floor']
export const houseoptions=[{name:'Condo',img:condo},{name:'House',img:house},{name:'Townhouse',img:townhouse}]
export const SizeOptions=['Less than 500','500 to 1,200 ','1,200 to 2,000 ','2,000 to 3,000 ','3,000 to 4,000 ','4,000 to 5,000 ']
export const SizeOptionsPressurewash=['0 to 1,000','1,001 to 1,500','1,501 to 2,000','2,001 to 2,500','2,501 to 3,000']
export const Timelist=[
    '5:00 AM',
    '6:00 AM',
    '7:00 AM',
    '8:00 AM',
    '9:00 AM',
    '10:00 AM',
    '11:00 AM',
    '12:00 Noon',
    '1:00 PM',
    '2:00 PM',
    '3:00 PM',
    '4:00 PM',
    '5:00 PM',
    '6:00 PM',
    '7:00 PM',
    '8:00 PM',
    '9:00 PM',
]
export const mainreviews=[{
    movingreview : [
    {
      name: 'Barry M',
      date:'22nd October 2022',
      review: "We had to move the contents of my mother-in-law's home after her passing. She was located in a rural area on Vancouver Island. Erick and Victor were...",
      rating:4.9
    },
    {
      name: "Alison Rasberry",
      date:'12th August 2022',
      review: "I had a very very positive experience with Lions Gate Moving last weekend. After many bad movers experiences before that in my life. This was the best by far. I was moving within the city from a one-bedroom sunken basement suite to an apartment building and was given a very reasonable quote of three movers (so it would be fast and efficient).",
      rating:5
    },
    {
      name: "Brittany Barker",
      date:'11th September 2022',
      review: "Collected a bunch of quotes from different moving companies and went with Scott and Steven @Lions Gate Moving because of the initial conversation and reasonable quote. Friendly, communicative, small moving business. Showed up on time, with all the supplies and moved all my stuff without incident or damage!",
      rating:4.8
    },
    {
      name: "Michelle Chand",
      date:'8th October 2022',
      review: " I used this moving company in late december with a lot snow on the ground. Alice accommodated my move after I required a few changes just few days before the move date. The movers were friendly, efficient, professional and worked great as a team 👏 I would highly recommend this company to anyone requiring moving services.",
      rating:4.5
    },
    {
      name: "Agnes Kot",
      date:'17th September 2022',
      review: " I cannot say enough about how hardworking and positive the crew were when moving me. I was really having a stressful time with having to downsize and move after many years at my previous home. From the moment they came through the door the members of the crew reassured me and told me not to worry and that they were there to take care of me. They took very good care of me and of my mountain of belongings.",
      rating:4.7
    },
    {
        name: 'Kaela Capadouca ',
        date:'22nd October 2022',
        review: "I hired them to move a 170 gallon fish tank and steel stand, they were awesome.They wrapped the tank up in a blanket and shrink wrap to protect the corners and did the same with the stand. They made a mini ramp to lift the tank up onto the deck and once inside the house they measured there space margins to ensure nothing would be damaged. ",
        rating:4.9
      },
      {
        name: "Sandra Brice",
        date:'12th August 2022',
        review: "The crew was professional and took care with all my belongings and they worked quickly and were on time and courteous. I have a second move booked and I am confident it will go just as smoothly. All communication has been clear and my inquiries have been responded to quickly. I would not hesitate to recommend them. My second move went very smoothly. The crew worked quickly and were very thoughtful. Thanks guys. ",
        rating:5
      },
      {
        name: "Sarah Zaharia",
        date:'11th September 2022',
        review: "Our moving truck arrived right on time and the move was even faster than we were quoted! A very smooth day. I'd highly recommend Real Pro for anyone who's moving. On a day that can be very stressful, we were in good hands and relaxed. ",
        rating:4.8
      },
      {
        name: "Jacqueline Murtough ",
        date:'8th October 2022',
        review: "Ryne was a great help in organizing my move. His team helped move a large oversized machine for us and also packed up our house into a sea-can and a cargo trailer. They worked hard and efficiently. Definitely recommend! ",
        rating:4.5
      },
      {
        name: "Elspeth Fisher ",
        date:'17th September 2022',
        review: " The guys were brilliant! Professional, reliable and friendly. They helped with a small down the street move and did it in as fast a time as possible. Everything arrived safely and was wrapped as part of their service. Would highly recommend! ",
        rating:4.7
      },
  ],
   carpetreview :[
    {
      name: 'Andres Bonnett',
      date:'22nd October 2022',
      review: "Felipe and the SanaClean team did an exceptional, thorough and detailed job on our first home. We called on Monday, and they were able to accommodate us on the Wednesday of the same week. The amount of work they completed in such a short period of time is incredible. It feels like we’re moving into a brand new condo, I could not thank them enough! ",
      rating:4.8
    },
    {
      name: "Mark Bradford",
      date:'12th August 2022',
      review: "Positive: Professionalism, Punctuality, Quality, Value Felipe and his crew were excellent to deal with. He was quick to respond to our inquiry, provided an accurate quote, arrived in a timely manner and did exactly what was required. We couldn't ask for anything more. Highly recommended for anyone wanting professional cleaning services. Two big thumbs up. ",
      rating:5
    },
    {
      name: "P.J. M ",
      date:'11th September 2022',
      review: "Positive: Professionalism I would like to complement Nicki on the fantastic job revamping my sectional. When I walked into my living room and looked at my once terribly stained sectional I was amazed at the transformation. I was never expecting it to come out THAT clean. Absolutely incredible service, positive attitude and attention to detail - Nicki is the total package!! I’m adding your number to my speed dial:) ",
      rating:4.8
    },
    {
      name: "Maria Ansorena ",
      date:'8th October 2022',
      review: " Positive: Professionalism,Punctuality,Quality,Value Nicki did a great job at our house. Very professional, fast, and efficient. Adding to being on time and very nice. We definitely recommend this company.",
      rating:4.9
    },
    {
      name: "Mary D. Manning ",
      date:'17th September 2022',
      review: "He was timely, cost-effective PLUS his attentive care to the carpets AND furniture made this a very good experience. Try them! I am very pleased." ,
      rating:5
    },
    {
        name: 'Preeti Parmar ',
        date:'22nd October 2022',
        review: "Very reliable & reasonable rates from Gary. I have referred definitely your service to many of my friends and co-workers. Absolutely praise their Eco-Giants quality & product line they use in their cleaning services. Gary is on amazing gentleman to ensure the job gets completed and executed with efficiency. ",
        rating:4.8
      },
      {
        name: "Aman Manhas",
        date:'12th August 2022',
        review: "We’ve hired Cleaning on several occassions and are extremely impressed with their work ethic, professionalism, and high quality level of work! They are our go-to for any cleaning services we need. If you’re looking for a highly skilled, efficient and knowledgeable company for your cleaning needs, you need to contact them immediately. We look forward to working with you again! ",
        rating:5
      },
      {
        name: "farouk messaikeh ",
        date:'11th September 2022',
        review: "He arrived on time, great service super professional, and got my carpet back to new, it was in a very bad shape. Would definitely recommend. 👌🏻👌🏻 Great work ",
        rating:4.8
      },
      {
        name: "Annie Baric ",
        date:'8th October 2022',
        review: " Joe and his team were a pleasure to work with. Joe accommodated our dates and times and made the entire process as easy as it could be. Austin arrived on time and was friendly and accommodating. Our carpets look fantastic. We definitely plan on using them again. ",
        rating:4.9
      },
      {
        name: "Cyrielle Leon ",
        date:'17th September 2022',
        review: "Joe and his team were the best. They provided a free (and in person) quote. They accommodated our tight timeline. They showed up right on time. They were so polite. And the service itself was amazing. Our entire office was left spotless and pristine clean. Highly recommended  Absolute professionals. " ,
        rating:5
      },
  ],
    plumbingreview :[
    {
      name: 'Natalie C. Khan',
      date:'24nd October 2022',
      review: "Great job with installation of new hot water tank. Arrived on schedule, left no mess, competitive pricing. Highly recommend !! Thank you Joseph ",
      rating:4.8
    },
    {
      name: "Jes Gölbez",
      date:'16th August 2022',
      review: `D&D was recommended by a friend, who expressed approval of their services. We've now used them twice, and they also helped one my friends with some bathroom repairs. D&D came to our place quickly to deal with an emergency, even after hours, explained the situation in layman's terms, and didn't charge an arm and a leg. The fellow also cleaned up well after himself. I would definitely use them again.`,
      rating:5
    },
    {
      name: "Michael J. ",
      date:'28th November 2022',
      review: "I had a leak from the bottom of my toilet. Jass was great, prompt, efficient and professional. He replaced the old wax ring and was finished within an hour. I would recommend Nifty Plumbing and will use them again in the future. ",
      rating:4.8
    },
    {
      name: "Adaku Emeka ",
      date:'12th October 2022',
      review: "I highly recommend Tiptop Plumbing!! Kevin was responsive and did an amazing work repairing our heating system. Thanks Tiptop for such great customer service especially for loaning us your single heaters while you work on our system. You're so thoughtful. I wish the had more than 5 stars because you deserve it. ",
      rating:4.9
    },
    {
      name: "Matt Spoli ",
      date:'19th September 2022',
      review: "I recently moved from Ontario and Kevin replaced my water heaters (i have 2) here in Calgary. One new heater was faulty out of the box. He quickly called the supplier, scheduled the swap and the next day came to replace the heater. Without charging me for the faulty equipment, it was hassle free. Amazing service, great price and honest work. Its TIP TOP service! Thanks Kevin!" ,
      rating: 5
    },
    {
        name: 'Sylvia R ',
        date:'24nd October 2022',
        review: "Very good service! We are just thankful for Paul to help us to repairing the water main leaking. He was so polite, patients and profession and solved our problem! Also, the service department girl Linna she is so friendly and helpful! Thank you for the excellent service! ",
        rating:4.8
      },
      {
        name: "NZR Productions ",
        date:'16th August 2022',
        review: `Great job from beginning to end! Alex provided an A1 service and the best communication all the the way through the process. I would highly recommend them to anyone who is looking for quality plumbing.`,
        rating:5
      },
      {
        name: "Spencer Perry ",
        date:'28th November 2022',
        review: "Great service , especially Brad, the plumber who showed up to fix our faucet and diverter. The dispatcher was professional and quick to schedule an appointment the same day. Brad was friendly, respectful of our home and finished the job quickly and with care. I love that they can easily take credit card payments. ",
        rating:4.8
      },
      {
        name: "Vicki Lee ",
        date:'12th October 2022',
        review: "Mike came and helped us fix the water heater because our hot water stopped running. He was punctual and super friendly and professional. He called around and asked if any warehouses had parts and went to go pick it up and as soon as he returned he was able to fix it right away. ",
        rating:4.9
      },
      {
        name: "Ross McCann ",
        date:'19th September 2022',
        review: "Great service and staff! Had them fix my toilet which had a backed up pipe and install several appliances. The work was done to a high standard and the price was very fair! And the cherry on the top is the boys are great for a chat while they are doing the work! " ,
        rating: 5
      },
  ],
   electriciansreview : [
    {
      name: 'Riyaz ',
      date:'26nd October 2022',
      review: "Electric Avenue came quickly when we had a problem with our electrical panel. The technicians explanation was clear and told me the cost possibilities up front. We would be happy to use them again. ",
      rating:4.8,
      nickName:'R'
    },
    {
      name: "Anmore ",
      date:'17th August 2022',
      review: `Highly recommend Tyler at Electronic Avenue! I reached out to him over Yelp and he responded right away. He called when he said he would, he showed up when he said he would. He did the job and charged the price he quoted! I am happy to support a small local business and now Tyler at Electric Avenue is my new electric guy for any of my electrical needs! Thanks so much Tyler! `,
      rating:5,
      nickName:'A'
    },
    {
      name: "Port Coquitlam ",
      date:'28th November 2022',
      review: "Tyler is Very Prompt to Respond, Thorough & Methodical. He explains everything upfront, along with costs - NO SURPRISES!. We tracked the source of the short circuit. He reattached an exposed ground wire and had the lights and door bell working in an hour. Everything works Perfectly Now. FYI, he prefers Cash, Interac, or e-Transfers and issues a Receipt on the Spot - Done ✅. I would use Tyler again, knowing how he works and interacts. I paid what I expected and he was competitively priced for the Service. ",
      rating:4.8,
      nickName:'PC'
    },
    {
      name: "Maple Ridge ",
      date:'12th October 2022',
      review: "Tyler came in shortly after initial calland completed the repair. He also gave us some hints on how to use the electrical appliances better and what to do when we encounter the same problem. Very satisfied customer. ",
      rating:4.9,
      nickName:'MR'
    },
    {
      name: "Jules H. ",
      date:'19th September 2022',
      review: "If someone shows up and does the job that's half the battle these days and Nightingale did both... they were later than expected due to a problem with their previous call but kept me informed.  They aren't inexpensive, 250.00 for an hour call but I did appreciate Garrett saying that if there was anything else I needed addressed, or smoke detectors checked, I may as well as I'm paying for an hour anyway. Could have just fixed and run ;)  would definitely use them again.. " ,
      rating: 5,
      nickName:'JH'
    },
    {
        name: 'Isaac Asuncion',
        date:'26nd October 2022',
        review: "Can’t recommend Electrician Sean enough. Very knowledgeable, friendly and professional. He was so good servicing our house that we specifically asked for him for another property of ours. Also, the customer reps answering the phones were friendly, helpful and professional. I give this company 4.5 out of 5 and yes, that’s a good thing ",
        rating:4.8,
        nickName:'IA'
      },
      {
        name: "Terry Hansen ",
        date:'17th August 2022',
        review: `I couldn't be happier with the high quality installation of wiring from fuse panel to the garage with many lights, switches, and outlets.
        Very knowledgeable, courteous, and professional service.
        I highly recommend this company. Brad was exceptionally knowledgeable, courteous, and helpful with ideas to make the job work out correctly. Shawn was also a great assistant.  `,
        rating:5,
        nickName:'TH'
      },
      {
        name: "Sylas Vallee",
        date:'28th November 2022',
        review: "I am an office mover and require electrician assistance from time to time. I have used Kato for multiple projects and have always received excellent services sometimes with very short notice",
        rating:4.8,
        nickName:'SV'
      },
      {
        name: "Robby Simpson ",
        date:'12th October 2022',
        review: "Mani was excellent. Managed to fix electrical issues for 5 suites under budget and under the expected time.  ",
        rating:4.9, nickName:'RS'
      },
      {
        name: "June Lai ",
        date:'19th September 2022',
        review: "My was so wonderful and responsive! He gave me great advice to help me save money on his services and appliances, and even came out on short notice. You can tell he knows what he's doing and he cares about his clients. I highly recommend him for anyone! " ,
        rating: 5,
        nickName:'JL'
      },
  ],
   pressurereview : [
    {
      name: 'Elizabeth R.',
      date:'26nd October 2022',
      review: "I highly recommend Quality Pressure Wash. They did an awesome job cleaning windows and dusting cobwebs at my home and business office. They are very professional and have great pricing.  ",
      rating:4.8,
      nickName:'ER'
    },
    {
      name: "Anthony N.",
      date:'17th August 2022',
      review: `Quality Pressure Wash's owner was extremely professional and courteous. He pressure washed our garage and backyard and cleaned the windows inside and out. He is a true professional.`,
      rating:5,
      nickName:'AN'
    },
    {
      name: "Brian S.",
      date:'28th November 2022',
      review: "The company offered a free estimate and recommendations and provided a detailed list of services and reminders which were super helpful! The team was very punctual and dependable! My house, driveway and porch look great!  ",
      rating:4.8,
      nickName:'BS'
    },
    {
      name: "Danny H.",
      date:'12th October 2022',
      review: "My whole experience with QPW was excellent, from beginning to end. My roof and my house were both soft washed. What a difference! What's best was that I felt they cared just as much about me as about the job. ",
      rating:4.9,
      nickName:'DH'
    },
    {
      name: "Susan Randhawa",
      date:'19th September 2022',
      review: "I absolutely recommend Quality Pressure Wash. They were very thorough in their work, going above and beyond to clean areas I hadn't even considered. I am very happy with the results and all at quite reasonable rates/price. Would definitely use again." ,
      rating: 5,
      nickName:'SR'
    },
    {
        name: 'Nick Burgess',
        date:'26nd October 2022',
        review: "Chris and his crew did a fantastic job today! Our roof looks brand new, the house is spotless and sparkling clean. Chris was immediately responsive to my online inquiry, while almost all of his competitors were not. He followed up, was very personable and professional, communicated frequently and proactively, showed up on time, and exceeded our expectations! Thank you  team! We’ll be using you again for sure and highly recommend you",
        rating:4.8,
        nickName:'NB'
      },
      {
        name: "Giuseppe Stanciulescu ",
        date:'17th August 2022',
        review: `Chris and his team performed a roof, house, gatters, and windows wash/cleaning. Not only did they performed a great job but, they also pointed out and corrected some issues related to the property. They were very professional, fast and well organized. It was a pleasure to see how the job turn out at the end. For sure I will kip their contact for future work.`,
        rating:5,
        nickName:'GS'
      },
      {
        name: "Tony Fera",
        date:'28th November 2022',
        review: "We were very satisfied with the service provided. Both Chris and his colleague were very professional and answered all our questions and concerns. We were very satisfied with the final results. Thank you very much for a job well done. ",
        rating:4.8,
        nickName:'TF'
      },
      {
        name: "Bernard Loh ",
        date:'12th October 2022',
        review: "Chris did a very good job in cleaning my roof. He also cleared up the debris that was washed down from the roof and left my place cleaned. Highly recommended!! ",
        rating:4.9,
        nickName:'BL'
      },
      {
        name: "Derek Mullan",
        date:'19th September 2022',
        review: "I made an appointment on line and the crew arrived on time. They cleaned the debris inside the gutters and washed the outside as well so they looked new. A few hangers were off and they hooked them back on. " ,
        rating: 5,
        nickName:'DM'
      },
  ]
}]


export const RoomData=[
  { label: '1', value: '1', key: 1 },
  { label: '2', value: '2', key: 2 },
  { label: '3', value: '3', key: 3 },
  { label: '4', value: '4', key: 4 },
  { label: '5', value: '5', key: 5 },
  { label: '6', value: '6', key: 6 },
  { label: '7', value: '7', key: 7 },
  { label: '8', value: '8', key: 8 },
  { label: '9', value: '9', key: 9 },
  { label: '10', value: '10', key: 10 },
  { label: '11', value: '11', key: 11 },
  { label: '12', value: '12', key: 12 },
  { label: '13', value: '13', key: 13 },
  { label: '14', value: '14', key: 14 },
  { label: '15', value: '15', key: 15 },
  { label: '16', value: '16', key: 16 },
  { label: '17', value: '17', key: 17 },
  { label: '18', value: '18', key: 18 },
  { label: '19', value: '19', key: 19 },
  { label: '20', value: '20', key: 20 },
  { label: '21', value: '21', key: 21 },
  { label: '22', value: '22', key: 22 },
  { label: '23', value: '23', key: 23 },
  { label: '24', value: '24', key: 24 },
  { label: '25', value: '25', key: 25 },
  { label: '26', value: '26', key: 26 },
  { label: '27', value: '27', key: 27 },
  { label: '28', value: '28', key: 28 },
  { label: '29', value: '29', key: 29 },
  { label: '30', value: '30', key: 30 },
  { label: '31', value: '31', key: 31 },
  { label: '32', value: '32', key: 32 },
  { label: '33', value: '33', key: 33 },
  { label: '34', value: '34', key: 34 },
  { label: '35', value: '35', key: 35 },
  
]
export const NumHallways=[
  { label: '1', value: '1', key: 1 },
  { label: '2', value: '2', key: 2 },
  { label: '3', value: '3', key: 3 },
  { label: '4', value: '4', key: 4 },
  { label: '5', value: '5', key: 5 },
]