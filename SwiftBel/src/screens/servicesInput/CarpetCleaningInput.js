...
import {  getCarpetCleaningDetails, getPressureWashDetails } from '../../store/actions/Profile.action'
import moment from 'moment'
import {  houseoptions, NumHallways, RoomData, Timelist } from './Components/listofFormData'
import RnIncrementDecrementBtn from '../../components/IncrementDecrementBtn/IncrementDercrementbtn'
import CalendarPicker from 'react-native-calendar-picker'
import BottomSheet from "@gorhom/bottom-sheet";
import { parseInt } from 'lodash'
const CarpetCleaningInput = props => {
...
set: "2 Room + 1 Hallway"
    ...
    const onselect = async () => {
        ...
        setIndicator(true)
            ...
            res = await dispatch(getCarpetCleaningDetails(values))
        
        if (res.status) {
           // handleSnapClose()
            props.navigation.navigate('getPrice',{type:"Carpet Cleaning"})
        }
        else {
            Alert.alert(
                "Error",
                ...
                <View style={{ flex: 1 }}>
                    <ScrollView contentContainerStyle={{ marginBottom: 150 }} showsVerticalScrollIndicator={false} scrollEventThrottle={16} keyboardShouldPersistTaps="always">
                            ...
                            <View style={[styles.PickerButton]}>
                                <View style={styles.PickerButtonContainer}>
                                    <View style={{ borderWidth: 1, padding: 8, width: 50, borderRadius: 8, backgroundColor: palette.smokeWhite }}>
                                        <Text style={styles.PickerButtontext}>{values?.additionalRoom}</Text>
                                    </View>
                                    <RnIncrementDecrementBtn
                                        minreq={2}
                                        max={7}
                                        handleClick={(item) => handlechange('numberOfRooms', item)}
                                        val={parseInt(values?.additionalRoom)}
                                    />
                                </View>
                            </View>
                        </View>
                        <InputPicker
                          ...
                          headerName={'Number of stairs'}
                          modalHeader={'Number of stairs'}
                          PickerContainerStyle={{ width: '95%', marginBottom: 10,alignSelf:'center' }}
                          data={RoomData}
                          value={values.numberofStair}
                          onChange={(value) => {
                              handlechange("numberofStair", value)
                          }}
                        />
                        <InputPicker
                          buttonStyle={{ width: "95%" }}
                          headerName={'Number of hallways'}
                          modalHeader={'Number of hallways'}
                          PickerContainerStyle={{ width: '95%', marginBottom: 10,alignSelf:'center' }}
                          data={NumHallways}
                          value={values.additionalHall}
                          onChange={(value) => {
                              handlechange("additionalHall", value)
                          }}
                        />
...
CarpetCleaningInput.defaultProps = {
    data: [],
    onChange: () => { },
    onCancel: () => { },
    onselect: () => { },
...
export default CarpetCleaningInput
